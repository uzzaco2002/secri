...
public class TileEntityHealer extends TileEntity {
	...
	public void use(EntityPlayer player) {
		try {
			...
			storage = MaemeStorage.CardManager.getPlayerStorage((EntityPlayerMP) player);
			this.player = player;
			beingUsed = true;
			player.openGui(Maeme.instance, EnumGui.Healer.getIndex(), player.worldObj, 0, 0, 0);
			tickCount = 0;
			allPlaced = false;
			...
			if (!allPlaced && tickCount == ticksToPlace) {
				...
				for (int i = pokemonLastPlaced + 1; i < storage.partyPokemon.length; i++) {
					if (storage.partyPokemon[i] != null) {
						...
						try {
							if (MaemeStorage.CardManager.getPlayerStorage((EntityPlayerMP) player).EntityAlreadyExists(
