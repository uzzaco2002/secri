...
public class TileEntityFossilCleaner extends TileEntity {
	...
	public void updateEntity() {
		if (itemInCleaner != -1) {
			if (timer > 0) {
				...
				if (!itemClean && MaemeItemsFossils.getItemFromIndex(itemInCleaner) instanceof ItemCoveredFossil) {
					itemInCleaner = ((ItemCoveredFossil) MaemeItemsFossils.getItemFromIndex(itemInCleaner)).cleanedFossil.itemID;
					itemClean = true;
	...
	public ItemFossil getFinishedFossil() {
		if (itemClean) {
			...
			if (worldObj instanceof WorldServer)
				((WorldServer) worldObj).getPlayerManager().flagChunkForUpdate(xCoord, yCoord, zCoord);
			return MaemeItemsFossils.getFossilFromIndex(itemId);
		}
		return null;
	}
