...
import maemesoft.entities.pokeballs.PokeballTypeHelper;
...
public class BlockFossilMachine extends BlockContainer {
	...
	public ItemStack getPickBlock(MovingObjectPosition target, World world, int x, int y, int z) {
		return new ItemStack(MaemeItemsFossils.fossilMachineItem, 1);
	}
	...
	public void capturePokemonInMachine(World world, int x, int y, int z, EntityPlayer player, int side, float par7, float par8, float par9) {
		...
		EntityMaeme p = (EntityMaeme) MaemeEntityList.createEntityByName(tile.currentPokemon, world);
		if (tile != null)
			if (EnumPokemon.hasPokemon(tile.currentPokemon) && !EnumTrainers.has(tile.currentPokemon)) {
				...
				p.caughtBall = (((ItemPokeBall) MaemeItemsPokeballs.getItemFromID(tile.currentPokeball)).type);
				p.clearAttackTarget();
				if (tile.isShiny)
					p.setIsShiny(true);
				PokeballTypeHelper.doAfterEffect((((ItemPokeBall) MaemeItemsPokeballs.getItemFromID(tile.currentPokeball)).type), p);
				try {
					MaemeStorage.CardManager.getPlayerStorage((EntityPlayerMP) player).addToParty(p);
				} catch (Exception e) {
				}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float par7, float par8, float par9) {
		...
		if (tile.currentPokeball != -1 && player.getCurrentEquippedItem() != null && !(player.getCurrentEquippedItem().getItem() instanceof ItemFossil)) {
			...
			Item item = MaemeItemsPokeballs.getItemFromID(itemId);
			if (item == null) {
		...
		if (tile.currentFossil != -1 && player.getCurrentEquippedItem() != null && !(player.getCurrentEquippedItem().getItem() instanceof ItemPokeBall)) {
			...
			Item item = MaemeItems.getFossilItem(itemId);
			if (item == null) {
	...
	public int idDropped(int par1, Random par2Random, int par3) {
		return MaemeItemsFossils.fossilMachineItem.itemID;
	}
