...
public class BlockAnvil extends BlockContainer {
	...
	public int idDropped(int par1, Random par2Random, int par3) {
		return MaemeItems.anvilItem.itemID;
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float par7, float par8, float par9) {
		...
		if (((TileEntityAnvil) world.getBlockTileEntity(x, y, z)).itemOnAnvil != -1) {
			...
			Item item = MaemeItemsPokeballs.getItemFromID(itemId);
			if (item == null) {
				if (itemId == MaemeItems.aluminiumPlate.itemID)
					item = MaemeItems.aluminiumPlate;
				else if (itemId == MaemeItems.aluminiumIngot.itemID)
					item = MaemeItems.aluminiumIngot;
			}
		...
		if (player.getCurrentEquippedItem() != null
				&& (player.getCurrentEquippedItem().getItem() instanceof ItemPokeballDisc
						|| player.getCurrentEquippedItem().getItem() == MaemeItemsPokeballs.ironDisc || player.getCurrentEquippedItem().getItem() == MaemeItems.aluminiumIngot)) {
