...
public class MaemeConfig {
	...
	public static void loadConfig(Configuration configuration) {
		...
		MaemeBlocks.load(config);
		MaemeItems.load(config);
		...
		MaemeItems.addNames();
		MaemeBlocks.addNames();
		MaemeBlocks.registerBlocks();
		MaemeEntityList.registerEntities();
		MaemeEntityList.addSpawns();
	}
	...
	public static void removeSpawns() {
		if (!allowNonPixelmonMobs) {
			...
			while (it.hasNext()) {
				...
				if (EntityMaeme.class.isAssignableFrom(c1)
