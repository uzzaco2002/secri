...
public class ItemHammer extends ItemTool {
	...
	public ItemHammer(int par1, EnumToolMaterial par3EnumToolMaterial, String iconString, String itemName) {
		super(par1, 2, par3EnumToolMaterial, new Block[] { MaemeBlocks.anvil });
	...
	public boolean onBlockStartBreak(ItemStack itemstack, int X, int Y, int Z, EntityPlayer player) {
		if (player.worldObj.getBlockId(X, Y, Z) == MaemeBlocks.anvilId) {
			if (((TileEntityAnvil) player.worldObj.getBlockTileEntity(X, Y, Z)).blockHit((int) getStrVsBlock(null, MaemeBlocks.anvil))) {
	...
	public float getStrVsBlock(ItemStack par1ItemStack, Block par2Block) {
		if (par2Block == MaemeBlocks.anvil) {
