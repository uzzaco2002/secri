	...
	public void processCommand(ICommandSender var1, String[] var2) {
		try {
			...
			if (EnumPokemon.hasPokemon(var5)) {
				Entity var6 = MaemeEntityList.createEntityByName(var5, world);
				var6.setPosition(cc.posX, cc.posY + 1, cc.posZ);
				if (var2.length > 1)
					for (String s : var2) {
						if (s.equalsIgnoreCase("s"))
							((EntityMaeme) var6).setIsShiny(true);
						else if (s.startsWith("boss")) {
							if (s.endsWith("1")) ((EntityMaeme) var6).setBoss(EnumBossMode.Uncommon);
							else if (s.endsWith("2")) ((EntityMaeme) var6).setBoss(EnumBossMode.Rare);
							else if (s.endsWith("3")) ((EntityMaeme) var6).setBoss(EnumBossMode.Legendary);
						}
					}
