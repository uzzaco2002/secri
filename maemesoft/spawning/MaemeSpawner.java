...
import maemesoft.spawning.spawners.*;
import cpw.mods.fml.common.ITickHandler;
import cpw.mods.fml.common.TickType;

public class MaemeSpawner implements ITickHandler {
	...
	private static boolean countEntities(WorldServer world) {
		...
		for (int var3 = 0; var3 < world.loadedEntityList.size(); ++var3) {
			...
			for (SpawnerBase b : spawners) {
				if (var4 instanceof EntityMaeme) {
					if (((EntityMaeme) var4).pokemonLocation == b.spawnLocation)
						b.count++;
						...
						if (s.canPokemonSpawnHere(world, cpXtmp, cpYtmp, cpZtmp)) {
							...
							if (world.getClosestPlayer((double) x, (double) y, (double) z, 24.0D) == null) {
								...
								if (d >= 576.0F) {
									...
									try {
										pokemon = MaemeEntityList.createEntityByName(pokemonName, world);
									} catch (Exception e) {
										e.printStackTrace();
										return 0;
									}
									...
									if (pokemon instanceof EntityMaeme)
										((EntityMaeme) pokemon).pokemonLocation = spawnLocation;
									...
									if (pokemon.getCanSpawnHere()) {
										...
										if (maxInChunk == -1) {
											if (world.rand.nextInt(300) == 0 && pokemon instanceof EntityMaeme) {
												EntityMaeme pixelmon = (EntityMaeme) pokemon;
