...
public class EntityPokeBall extends EntityThrowable {
	...
	public EntityPokeBall(World world, EntityLiving entityliving, EntityMaeme e, EnumPokeballs type) {
		super(world, entityliving);
		thrower = entityliving;
	...
	protected void onImpact(MovingObjectPosition movingobjectposition) {
		...
		if (dropItem && breakChance == 1 && EnumPokeballs.getFromIndex(dataWatcher.getWatchableObjectInt(10)) != EnumPokeballs.MasterBall) {
			...
			entityDropItem(new ItemStack(MaemeItemsPokeballs.ironBase), 0.0F);
		...
		if (isBattleThrown && !worldObj.isRemote) {
			...
			if (movingobjectposition != null) {
				if (movingobjectposition.entityHit != null && (movingobjectposition.entityHit instanceof EntityMaeme)) {
					EntityMaeme EntityMaeme = (EntityMaeme) movingobjectposition.entityHit;
					p = EntityMaeme;
	...
	public Item breakBall() {
		return MaemeItemsPokeballs.getLidFromEnum(getType());
	}
		...
		if (getIsCaptured()) {
			if (waitTime > 20) {
				try {
					...
					MaemeStorage.CardManager.getPlayerStorage((EntityPlayerMP) thrower).addToParty(p);
					p.catchInPokeball();
	...
	private void catchPokemon() {
		if (canCatch) {
			...
			MaemeEventHandler.fireEvent(EventType.CapturePokemon, (EntityPlayer) thrower);
			spawnCaptureParticles();
			setIsCaptured(true);
			waitTime = 0;
		} else {
	...
	protected void doCaptureCalc(EntityMaeme p2) {
