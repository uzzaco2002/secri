...
public abstract class PixelmonInteraction {
	...
	EntityMaeme pixelmon;
	int maxInteractions;
	int maxCount = 400;

	public PixelmonInteraction(EntityMaeme pixelmon, int maxInteractions, boolean isFirstInteraction) {
	...
	public static PixelmonInteraction getInteraction(Entity8HoldsItems pixelmon, boolean isFirstInteraction) {
		...
		try {
			PixelmonInteraction p = (PixelmonInteraction) c.getConstructor(new Class[] { EntityMaeme.class, boolean.class }).newInstance(
					new Object[] { (EntityMaeme) pixelmon, isFirstInteraction });
			return p;
		} catch (Exception e) {
			e.printStackTrace();
		}
