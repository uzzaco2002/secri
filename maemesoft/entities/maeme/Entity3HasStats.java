...
import maemesoft.entities.maeme.stats.BaseStats;
import maemesoft.entities.maeme.stats.FriendShip;
import maemesoft.entities.maeme.stats.IVStore;
import maemesoft.entities.maeme.stats.Level;
import maemesoft.entities.maeme.stats.Stats;
...
public abstract class Entity3HasStats extends Entity2HasModel {
	...
	public Entity3HasStats(World par1World) {
		...
		stats = new Stats();
		level = new Level((EntityMaeme) this);
		friendship = new FriendShip((EntityMaeme) this);
	...
	private BaseStats loadBaseStats(String name) {
		BaseStats store = DBStats.GetBaseStats(name);
		return store;
	}
	...
	public void onDeath(DamageSource par1DamageSource) {
		if (getOwner() != null) {
			friendship.onFaint();
			MaemeEventHandler.fireEvent(EventType.PokemonFaint, (EntityPlayer) getOwner());
		}
	...
	public void moveEntityWithHeading(float par1, float par2) {
		if (baseStats != null) {
			if (((EntityMaeme) this).pokemonLocation == SpawnLocation.Water && isInWater()) {
	...
	protected void fall(float f) {
		if (baseStats != null) {
			if (((EntityMaeme) this).pokemonLocation == SpawnLocation.Water)
				return;
			if (baseStats.canFly)
				return;
		}
	...
	public boolean canBreatheUnderwater() {
		if (baseStats != null) {
			if (((EntityMaeme) this).pokemonLocation == SpawnLocation.Water || baseStats.type1 == EnumType.Water || baseStats.type2 == EnumType.Water)
				return true;
			else
				return false;
		}
