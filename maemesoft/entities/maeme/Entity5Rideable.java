...
import maemesoft.entities.maeme.helpers.RidingHelper;
...
public abstract class Entity5Rideable extends Entity4Textures {
	...
	protected void init(String name) {
		...
		if (baseStats.isRideable && worldObj.isRemote)
			ridingHelper = new RidingHelper((EntityMaeme) this, worldObj);
	}
			...
			if (MaemeConfig.allowRiding) {
				...
				if (itemstack == null) {
					if (baseStats.isRideable && belongsTo(player)) {
						...
						MaemeEventHandler.fireEvent(EventType.RidePokemon, player);
						return true;
					}
					return false;
				}
