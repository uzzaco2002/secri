...
public class GuiScreenPokeChecker extends GuiContainer {
	protected MaemeDataPacket targetPacket;
	GuiButton nameButton;
	boolean renameButton;
	boolean isPC;

	public GuiScreenPokeChecker(MaemeDataPacket pixelmonDataPacket, boolean b) {
	...
	public void drawGuiContainerBackgroundLayer(float f, int i, int i1) {
		...
		mc.renderEngine.bindTexture("/maemesoft/gui/summarySummary.png");
		GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
	...
	private void drawExpBar(int x, int y, int width, int height, MaemeDataPacket p) {
	...
	public void drawHealthBar(int x, int y, int width, int height, MaemeDataPacket p) {
		GL11.glEnable(GL12.GL_RESCALE_NORMAL);
		GL11.glEnable(GL11.GL_COLOR_MATERIAL);
		GL11.glPushMatrix();
		Tessellator tessellator = Tessellator.instance;
