...
public class GuiMaemeOverlay extends Gui {
		...
		for (MaemeDataPacket p : ServerStorageDisplay.pokemon) {
			...
			if (p != null) {
				...
				if (p.isShiny)
					...
					Minecraft.getMinecraft().renderEngine.bindTexture("/mods/Maeme/sprites/pokemon/" + numString + ".png");
	...
	private void bindOverlayTexture() {
		if (isGuiMinimized)
			Minecraft.getMinecraft().renderEngine.bindTexture("/maemesoft/gui/pixelmonOverlaySimple.png");
		else
	...
	public boolean checkEntitysInWorld(World world) {
		...
		for (int i = 0; i < EntityList.size(); i++) {
			...
			if (e instanceof EntityMaeme) {
				int existingId = ((EntityMaeme) e).getPokemonId();
				if (existingId != -1) {
					for (MaemeDataPacket p : ServerStorageDisplay.pokemon) {
						if (p != null) {
							if (((EntityMaeme) e).getPokemonId() == p.pokemonID) {
								p.outside = true;
							}
						}
					}
