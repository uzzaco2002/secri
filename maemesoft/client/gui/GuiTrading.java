...
public class GuiTrading extends GuiContainer {
	...
	protected void drawGuiContainerBackgroundLayer(float var1, int var2, int var3) {
		...
		mc.renderEngine.bindTexture("/maemesoft/gui/tradeGui.png");
		...
		for (MaemeDataPacket p : ServerStorageDisplay.pokemon) {
			...
			if (p != null) {
				...
				if (p.isShiny)
					Minecraft.getMinecraft().renderEngine.bindTexture("/maemesoft/sprites/shinypokemon/" + numString + ".png");
				else
					Minecraft.getMinecraft().renderEngine.bindTexture("/maemesoft/sprites/pokemon/" + numString + ".png");
		...
		if (selected != -1 && ServerStorageDisplay.pokemon[selected] != null) {
			MaemeDataPacket p = ServerStorageDisplay.pokemon[selected];
			String displayName = p.name;
			if (!p.nickname.equals(""))
				displayName = p.nickname;
		...
		if (ClientTradingManager.tradeTarget != null) {
			MaemeDataPacket p = ClientTradingManager.tradeTarget;
			String displayName = p.name;
			if (!p.nickname.equals(""))
				displayName = p.nickname;
	...
	MaemeDataPacket[] p = ServerStorageDisplay.pokemon;
