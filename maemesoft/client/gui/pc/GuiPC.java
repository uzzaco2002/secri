...
import maemesoft.client.MaemeServerStore;
...
public class GuiPC extends GuiContainer {
	...
	public GuiPC(MaemeDataPacket targetPacket) {
		this();
	...
	public void initGui() {
		...
		for (int i = 0; i < PlayerComputerStorage.boxCount; i++) {
			for (int j = 0; j < ComputerBox.boxLimit; j++) {
				...
				MaemeDataPacket p = null;

				p = MaemeServerStore.getFromBox(i, j);
		...
		for (int i = 0; i < 6; i++) {
			...
			MaemeDataPacket p = ServerStorageDisplay.pokemon[i];
			if (p != null) {
	...
	protected void mouseClicked(int par1, int par2, int par3) {
		...
		if (par3 == 0) {
			...
			if (slot != null) {
				...
				MaemeDataPacket temp = slot.pokemonData;
				if ((slot instanceof SlotPCParty && MaemeServerStore.getMousePokemon() == null && checkIfLast())) {
					return;
				}
				if (slot.pokemonData != null) {
					changed = true;
				}
				if (MaemeServerStore.getMousePokemon() != null) {
					changed = true;
				}
				if (changed) {
				...
				if (MaemeServerStore.getMousePokemon() != null) {
					...
					mc.displayGuiScreen(new GuiScreenPokeCheckerPC(MaemeServerStore.getMousePokemon(), 0, 0));
				}
				return;
	...
	public MaemeDataPacket mouseHeldPokemon = null;
	...
	public void onGuiClosed() {
		...
		GuiMaemeOverlay.checkSelection();
	}
	...
	protected void drawGuiContainerBackgroundLayer(float var1, int var2, int var3) {
		...
		SlotPC mouseSlot = new SlotPC(var2 - 15, var3 - 15, MaemeServerStore.getMousePokemon());
		RenderHelper.disableStandardItemLighting();
		GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);


		mc.renderEngine.bindTexture("/maemesoft/gui/pcPartyBox.png");
		drawTexturedModalRect(width / 2 - 91, height / 6 + 151, 0, 0, 182, 29);
		int i = 0;
		mc.renderEngine.bindTexture("/maemesoft/gui/pcBox.png");
		drawTexturedModalRect(width / 2 - 91, height / 6, 0, 0, 182, 141);
		drawTexturedModalRect(trashX, trashY, 0, 256 - 32, 32, 32);
		mc.renderEngine.bindTexture("/mods/Maeme/textures/items/pokechecker.png");
		...
		for (int a = 0; a < pcSlots[boxNumber].length; a++) {
			...
			if (slot.pokemonData.heldItemId != -1) {
				mc.renderEngine.bindTexture("/maemesoft/image/helditem.png");
		...
		if (mouseSlot.pokemonData != null) {
			MaemeDataPacket p = mouseSlot.pokemonData;
			String numString = "";
	...
	private void refreshSlots() {
		for (int i = 0; i < PlayerComputerStorage.boxCount; i++) {
			for (int j = 0; j < ComputerBox.boxLimit; j++) {
				MaemeDataPacket p = MaemeServerStore.getFromBox(i, j);
				pcSlots[i][j].setPokemon(p);
