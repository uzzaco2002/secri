...
import maemesoft.client.models.ModelFossilMachine;
...
public class RenderTileFossilMachine extends TileEntitySpecialRenderer {
	...
	public void renderAModelAt(TileEntityFossilMachine tile, double d, double d1, double d2, float f) {
		...
		bindTextureByName("/maemesoft/texture/blocks/fossilmachine.png");
		GL11.glPushMatrix();
		...
		if (tile.currentPokeball != -1) {
			bindTextureByName("/maemesoft/textures/pokeballs/" + tile.currentPokeballTexture);
		...
		if (pokemon == null || (pokemon != null && pokemon.getName() != tile.currentPokemon)){
			pokemon = ((EntityMaeme) MaemeEntityList.createEntityByName(tile.currentPokemon, tile.getWorldObj()));
	...
	public String fossilTexture(TileEntityFossilMachine tile) {
		if (tile.currentFossil != -1)
			return MaemeItemsFossils.getFossilFromIndex(tile.currentFossil).getModelName().toLowerCase();
		else
			return "";
	}
	...
	public void renderModel(TileEntityFossilMachine tile, float f) {
		if (tile.currentFossil != -1)
			if (MaemeItemsFossils.getFossilFromIndex(tile.currentFossil).getModel() != null)
				MaemeItemsFossils.getFossilFromIndex(tile.currentFossil).getModel().renderModel(f);
	}
