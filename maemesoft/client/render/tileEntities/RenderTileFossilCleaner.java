...
import maemesoft.client.models.ModelFossilCleaner;
...
public class RenderTileFossilCleaner extends TileEntitySpecialRenderer {
	...
	public void renderTileEntityAt(TileEntity tile, double d, double d1, double d2, float f) {
		...
		if (i < 0)
			return;
		if (Minecraft.getMinecraft().theWorld.getBlockId(tile.xCoord, tile.yCoord - 1, tile.zCoord) == MaemeBlocks.pc.blockID)
			return;
		...
		bindTextureByName("/maemesoft/texture/blocks/fossilcleaningmachine.png");
		if (tileEntity.isOn())
	...
	public String fossilTexture(TileEntityFossilCleaner tile) {
		return MaemeItemsFossils.getFossilFromIndex(tile.itemInCleaner).getModelName().toLowerCase();
	}
