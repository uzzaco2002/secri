...
import maemesoft.client.models.ModelHealer;
...
public class RenderTileEntityHealer extends TileEntitySpecialRenderer {
	...
	public void renderAModelAt(TileEntityHealer tile, double d, double d1, double d2, float f) {
		...
		bindTextureByName("/maemesoft/texture/blocks/healer.png"); // texture
	...
	private void renderPokeball(int k, TileEntityHealer tile) {
		...
		if (tile.allPlaced) {
			if (tile.flashTimer < 10 || tile.stayDark)
				Minecraft.getMinecraft().renderEngine.bindTexture("/maemesoft/textures/pokeballs/" + tile.pokeballType[k].getCaptureTexture());
			else
				Minecraft.getMinecraft().renderEngine.bindTexture("/maemesoft/textures/pokeballs/" + tile.pokeballType[k].getTexture());
