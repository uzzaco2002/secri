...
public class RenderMaeme extends RenderLiving {
	...
	private int defaultBossNameRenderDistanceExtension = 8; // kept as an
															...
															// avoid scaling
															// issues
	private int configNameRenderMultiplier = (int) Math.max(1, Math.min(MaemeConfig.nameplateRangeModifier, 3)); // keeps
																													// in
																													// bounds
																													// [1,
																													// 3],
	...
	public RenderMaeme(float par2) { // par2 = shadow size
		super(null, par2);
	}

	public void doRenderLiving(EntityLiving entityLiving, double d, double d1, double d2, float f, float f1) {
		EntityMaeme pixelmon = (EntityMaeme) entityLiving;
		if (pixelmon.getName().equals(""))
			return;
		pixelmon.init(pixelmon.getName());
		GL11.glAlphaFunc(GL11.GL_GREATER, 0.1f);
		if (pixelmon.getModel() != null)
			RenderMaeme(pixelmon, d, d1, d2, f, f1);
	...
	public void RenderMaeme(EntityMaeme pixelmon, double par2, double par4, double par6, float par8, float par9) {
		GL11.glPushMatrix();
	...
	public void drawNameTag(EntityMaeme EntityMaeme, double par2, double par4, double par6, boolean owned) {
		if (Minecraft.isGuiEnabled()) {
			String s = "";
			if (EntityMaeme.getBossMode() == EnumBossMode.Normal)
				s += " Lv: " + EntityMaeme.getLvl().getLevel() + " ";
			s += EntityMaeme.getNickname();
			if (EntityMaeme.getBossMode() == EnumBossMode.Normal) {
				if (EntityMaeme.hasOwner()) {
					s += " (" + EntityMaeme.getOwnerName() + ")";
				} else
					s += " (Wild)";
			}
			if (!EntityMaeme.isSneaking()) {
				renderLivingLabel(EntityMaeme, s, par2, par4, par6, owned);
			}
		}
	}

	protected void renderLivingLabel(EntityMaeme EntityMaeme, String par2Str, double par3, double par5, double par7, boolean owned) {
		double var10 = EntityMaeme.getDistanceSqToEntity(this.renderManager.livingPlayer);
		FontRenderer var12 = this.getFontRendererFromRenderManager();
		float var13 = 1.6F;
		float var14 = 0.016666668F * var13;
		GL11.glPushMatrix();
		float scaleFactor = MaemeConfig.scaleModelsUp ? 1.3f : 1;
		scaleFactor *= EntityMaeme.getScaleFactor();
		GL11.glTranslatef((float) par3 + 0.0F, (float) par5 + 1.1f + EntityMaeme.height * EntityMaeme.getScale() * scaleFactor, (float) par7);
		...
		GL11.glDepthMask(true);
		var12.drawString(par2Str, -var12.getStringWidth(par2Str) / 2, var16, EntityMaeme.getBossMode().getColourInt());
		GL11.glEnable(GL11.GL_LIGHTING);
		GL11.glDisable(GL11.GL_BLEND);
		GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
		GL11.glPopMatrix();
	...
	public void drawExpBar(EntityLiving entityLiving, double d, double d1, double d2, float f, float f1, boolean owned) {
		...
		if (Minecraft.isGuiEnabled()) {
			GL11.glPushMatrix();
			float scaleFactor = MaemeConfig.scaleModelsUp ? 1.3f : 1;
			scaleFactor *= ((EntityMaeme) entityLiving).getScaleFactor();
			GL11.glTranslatef((float) d + 0.0F, (float) d1 + entityLiving.height * ((EntityMaeme) entityLiving).getScale() * scaleFactor, (float) d2);
			GL11.glNormal3f(0.0F, 1.0F, 0.0F);
			...
			float f5 = ((EntityMaeme) entityLiving).getLvl().getExp();
			float f6 = ((EntityMaeme) entityLiving).getLvl().getExpToNextLevel();
			if (f5 >= f6)
				f5 = 56;
			float f8 = 50F * (f5 / f6);
	...
	public void drawHealthBar(EntityLiving entityLiving, double d, double d1, double d2, float f, float f1, boolean owned) {
		...
		if (Minecraft.isGuiEnabled()) {
			...
			GL11.glDisable(3553 /* GL_TEXTURE_2D */);
			tessellator.startDrawingQuads();
			float f5 = ((EntityMaeme) entityLiving).getHealth();
			float f6 = ((EntityMaeme) entityLiving).getMaxHealth();
			float f8 = 50F * (f5 / f6);
	...
	protected void preRenderScale(EntityMaeme entity, float f) {
		float scaleFactor = MaemeConfig.scaleModelsUp ? 1.3f : 1;
	...
	protected void preRenderCallback(EntityLiving entityliving, float f) {
		preRenderScale((EntityMaeme) entityliving, f);
	}
}